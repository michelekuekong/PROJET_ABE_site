...
if($saction=='Img'){
    ...
    if(!empty($ssaction)){
        // suppri,er l'image
        if($ssaction=='del'){
            $link=$this->getColsByCol('illustration_produit_prestation', 'lien', 'id', $id);
            $file='../'.$link;
            ...
            $param[0]['col']='online';
            $param[0]['val']=-1;
            $this->updateParamById('illustration_produit_prestation', $param, $id);

           header('Location:index.php?action=Produit_prestation&saction=Detail&id='.$id_prestation.'#bloc_file');
        }
            ...
            if(isset($_FILES)){
				...
				if(isset($_FILES['image']) && !empty($_FILES['image'])){
					if($_FILES['image']['error']==0 && $_FILES['image']['size'] <= $size){
                        $lien = $this->getColsByCol('illustration_produit_prestation', 'lien', 'id', $id);
                        $file='../'.$lien;
				...
				if(isset($_FILES['plaquette']) && !empty($_FILES['plaquette'])){
					if($_FILES['plaquette']['error']==0){
                        ...
                        $lien = $this->getColsByCol('illustration_produit_prestation', 'lien', 'id', $_GET['id']);
                        $file='../'.$lien;
