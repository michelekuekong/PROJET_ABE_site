			...
			<div class="card">
				<div class="card-body">
					...
					<div class="form-group col-12"   style="width: 100%;height:65px">
						<textarea  id="myeditor"  placeholder="<?php if(empty($resumé)){ echo 'Saisir votre resumé ici'; } ?>" name="resume" class="form-control"><?php if(!empty($resume )){ ?><?= trim($resume) ?><?php } ?></textarea> 
					</div>
					<div class="col-12" style="text-align:center; margin-top:50px;">
						<button  type="submit" class="btn btn-md btn-outline-primary" id="ts-success">
								Valider
...
<div class="card">
	<div class="card-body">
	<h5 class="card-title"> <strong>Illustration gallery</strong> </h5>
		<!-- image -->
		<?php if(!empty($img)){ ?>
	    <hr>
						...
						<div class="el-card-avatar el-overlay-1"> <img src="../<?= $value->getLien() ?>" style="width: 250px; height: 200px" alt="picture">
							<div class="el-overlay">
								<ul class="list-style-none el-info">
									...
									<li class="el-item"><a class="btn default btn-outline el-link" href="index.php?action=Illustration_gallery&saction=Img&id=<?= $value->getId()?>&ssaction=<?= ($value->getOnline()==1) ? 'putOffline' : 'putOnline' ?>" title="<?= ($value->getOnline()==1) ? 'Dépublier' : 'Publier' ?>"><i class="fa fa-eye <?= ($value->getOnline()==1) ? 'text-success' : 'text-warning' ?>"></i></a></li>
									<li class="el-item"><a  title="delete" class="btn default btn-outline video-popup-vertical-fit el-link" href="index.php?action=Illustration_gallery&saction=Img&ssaction=delete&id=<?= $value->getId() ?>" ><i class="fa fa-trash"></i></a></li>
					...
					<div class="modal-dialog modal-md" role="document">
						<div class="modal-content">
							...
							<div class="modal-body" style="padding:25px 25px 25px 25px; background-color:#F0F3F4">
								<form class="form-horizontal" method="POST" action="index.php?action=Illustration_gallery&saction=Img&ssaction=update&id=<?= $value->getId() ?>" enctype="multipart/form-data">
									...
									<!-- Create the editor container -->
									<div class="form-group col-12"   style="width: 100%;height:200px; margin-bottom: 20px;margin-top: 20px">
										<label for="detail"><strong>detail sur l'Image</strong></label>
										<textarea  id="myeditor"  placeholder="<?php if(empty($value->getDetail())){ echo 'Saisir votre detail ici'; } ?>" name="detail" class="form-control"><?php if(!empty($value->getDetail() )){ ?><?= trim($value->getDetail()) ?><?php } ?></textarea> 
									</div>
									<hr>
							...
							<div class="modal-content">
								...
								<div class="modal-body" style="padding:25px 25px 25px 25px; background-color:#F0F3F4">
									<form class="form-horizontal" method="POST" action="index.php?action=Illustration_gallery&saction=Img&ssaction=update&id=<?= $value->getId() ?>" enctype="multipart/form-data">
										...
										<!-- Create the editor container -->
										<div class="form-group col-12"   style="width: 100%;height:200px; margin-bottom: 20px;margin-top: 20px">
											<label for="detail"><strong>detail sur la video</strong></label>
											<textarea  id="myeditor"  placeholder="<?php if(empty($value->getDetail())){ echo 'Saisir votre detail ici'; } ?>" name="detail" class="form-control"><?php if(!empty($value->getDetail() )){ ?><?= trim($value->getDetail()) ?><?php } ?></textarea> 
										</div>
										<hr>
										<div class="col-md-8 text-right">
			...
			<form class="form-horizontal" method="POST" action="index.php?action=<?= $action ?>&saction=Detail&id=<?= $id?>&ssaction=update" enctype="multipart/form-data">
				<div class="form-group row">
						...
						<div  class="form-group col-12"   style="width: 100%;height:200px; margin-bottom: 20px; margin-top: 20px">
							...
							<textarea  id="myeditor"  placeholder="Saisir votre detail ici" name="detail" class="form-control"></textarea> 
						</div>
